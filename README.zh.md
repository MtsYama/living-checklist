...
![活页清单演示](assets/demo-zh-Hans.gif)

*勾一条它就收起,做完一整步卡片就折叠——全程自动存在浏览器里。*
...
「The Step-by-Step HTML」,我一般也这么叫它。
...
**这是什么(不是什么)。** 它不是一个装上就能用的成品 app,而是一种辅助 AI 生成更好的执行清单(step-by-step 清单)的 Skill / 插件(未来会升级):把需求说给 AI(ChatGPT、Claude 这些),或者自己改一下文件,它就产出一个 HTML 文件,双击就能用。工具本身不带 AI,靠你自己的 AI 来把清单生成出来。后面也许会长成一个能对接 Notion 那类工具的独立 app,但现在还没有;今天它就是这一个文件加一个 Claude skill。
...
**1. 自己动手改。** 复制一份模板，改里面的 `[1] DATA` 和 `[2] CONFIG` 两段，打开文件。不用构建。
...
字体：Noto、Cormorant Garamond、Alegreya、霞鹜文楷、IBM Plex Mono（都通过 Google Fonts 引入）。项目里用到的图标——落地页、模板和示例——均来自 [Phosphor](https://phosphoricons.com/)。
