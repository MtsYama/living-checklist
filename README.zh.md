...
- **「活的」**：
  - 勾选一条 → 它用 FLIP 动画平滑收进本步顶部的「✓ 已完成」分组（不想它动？工具栏点「勾完自动收起」关掉就行）；
  - 整步勾完 → 卡片挂上「✓ 已完成」徽标、就地折叠（不会被挪到页面底部）；
  - 顶部有总进度条；
  - **每个条目各带一个备注框**，自动保存，复制进度时一起带走，方便回头问 AI。
...
- **可折叠工具栏**：全部展开 / 全部折叠 / 重置勾选 / 勾完自动收起 / 「复制进度 + 反馈」（把当前进度和每条备注打包成 markdown 放到剪贴板，粘回 AI 对话里就能驱动下一轮迭代）。
...
默认模板提供两种干净配色——**黛蓝 / Lapis blue**（默认）和**酒红 / Burgundy**——外加一套 **MX Studio** 暗色设计师皮。模板都在 `templates/` 下。

| 模板 | 风格 | 默认主题 |
| --- | --- | --- |
| `base.html` | 干净的日历 App 风，浅色、克制，黛蓝点缀（系统黑体 / Noto）。默认通用模板。 | 自动 |
| `base-burgundy.html` | 同一套干净默认模板，换成暖调的酒红点缀。 | 自动 |
| `mx-studio.html` | 深色 noir + 金色，衬线字体（Cormorant Garamond + Alegreya + 霞鹜文楷 + IBM Plex Mono），大号 folio 步骤编号，Phosphor 图标。 | 深色 |

![mx-studio 模板，深色](assets/mx-zh-Hans.png)
...
![旅行例子](assets/example-zh-Hans.png)
