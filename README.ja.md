...
**中国系モデル（チャットモード）。** 豆包 / 通义千问 / 腾讯元宝 / 智谱清言（GLM） はいずれも HTML 全体を出力し、*かつ*インラインでプレビューします。Kimi はデプロイリンク経由でプレビューします。DeepSeek / 文心一言 / 讯飞星火 は HTML を出力しますがインラインプレビューはなく（`.html` で保存してダブルクリック）、長い HTML は途中で切れることがあります — 「続けて」と頼むか、分割して生成させてください。
