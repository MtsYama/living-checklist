...
「The Step-by-Step HTML」，我一般也這麼叫它。
...
**這是什麼（不是什麼）。** 它不是一個裝上就能用的成品 app，而是一種輔助 AI 產生更好的執行清單（step-by-step 清單）的 Skill / 外掛（未來會升級）：把需求說給 AI（ChatGPT、Claude 這些），或者自己改一下檔案，它就產出一個 HTML 檔案，雙擊就能用。工具本身不帶 AI，靠你自己的 AI 來把清單產生出來。後面也許會長成一個能對接 Notion 那類工具的獨立 app，但現在還沒有；今天它就是這一個檔案加一個 Claude skill。
...
字型：Noto、Cormorant Garamond、Alegreya、霞鶩文楷、IBM Plex Mono（都透過 Google Fonts 引入）。專案裡用到的圖示——落地頁、模板和範例——均來自 [Phosphor](https://phosphoricons.com/)。
